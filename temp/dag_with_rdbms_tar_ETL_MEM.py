...
def execute_query(hook, job, endpointUrl, bucketName, awsAccessKeyId, awsSecretAccessKey, num_rows_per_file, fileFormat, orient, Parameters):
...
def fn_oralceExport(jobSetting, jobList, **kwargs):
    ...
    for job in jobList:
        # logging.info("job['id'] ::"+job['id'])
        # logging.info("Parameters[queryId] ::"+Parameters['queryId'])
        if job['id'] == Parameters['queryId']:
            execute_query(hook, job, endpointUrl, bucketName, awsAccessKeyId, awsSecretAccessKey, num_rows_per_file, fileFormat, orient, Parameters)
...
with DAG(
    dag_id='locknlock_s3_2025_mig_mem',
    ...
    catchup = False,
    params={
        "queryId": Param("select_orderDetails", type="string", description="query id 를 입력하세요"),
        "queryparams": Param("USER_NAME  = 'admin' AND ROWNUM =1 AND LOGGED_TIME <= TO_DATE('2024-10-16') AND LOGGED_TIME <= TO_DATE('2024-10-16')", type="string", description="조건절을 입력하세요"),
    },
    tags = conf['tags']
) as dag:
